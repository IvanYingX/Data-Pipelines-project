...
def create_database(year_1, year_2, leagues):
    ...
    for league in leagues:
        df_standings.to_csv(f"Standings_Raw_{year_1}_{year_2}_{league}.csv", index=False)
        df_results.to_csv(f"Results_Raw_{year_1}_{year_2}_{league}.csv", index=False)
        for year in range(year_1, year_2 + 1):
            ...
            for round in range(1, num_rounds + 1):
                print(f'\tAccesing data from round {round} of year {year} of {league}') 
                ...
                pd.DataFrame(dict_results).to_csv(f"Results_Raw_{year_1}_{year_2}_{league}.csv",
                                mode='a', header=False, index=False)
                for key in dict_results:
                    dict_results[key].clear()

                pd.DataFrame(dict_standings).to_csv(f"Standings_Raw_{year_1}_{year_2}_{league}.csv",
                                mode='a', header=False, index=False)
