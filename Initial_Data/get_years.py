...
def get_initial_year():
        ...
        label = tk.Label(root, 
                ...
                justify = tk.LEFT, font=("Arial", 14), padx = 5, pady = 10).grid(row=0, column=1)
        slider = tk.Scale(root, from_= 1990, to = 2020, variable=v, orient=tk.HORIZONTAL, 
                        length=300, tickinterval=5, width=10).grid(row=1, column=1)
        tk.Radiobutton(root, text="Confirm", indicatoron = 0, width = 10, relief=tk.FLAT, value=True, command=root.destroy).grid(row=2, column=2, pady=10)
