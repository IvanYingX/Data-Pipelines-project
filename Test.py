...
for r in range(n_rounds):
    ...
    for _, row in df_standings.iterrows():
        ...
        if (cur_round['Home_Team'] == team).sum() == 1:
            ...
            cur_round['Position_Home'] = row['Position']
            ...
            # Let's write the performance of the home team when it is home
            cur_round.loc[idx, ['Wins_When_Home']] = int(row['Win_Home'])
            cur_round.loc[idx, ['Draw_When_Home']] = int(row['Draw_Home'])
            cur_round.loc[idx, ['Lose_When_Home']] = int(row['Lose_Home'])
            ...
            # Streak of the home team only for the matches played at home
            cur_round.loc[idx, ['Streak_When_Home']] = row['Streak_When_Home']

        # Let's write the performance of the Away team when it plays Away
        else:
            idx = cur_round.loc[cur_round['Away_Team']
                                == team].index.values[0]

            # First the position of the team
            cur_round['Position_Away'] = row['Position']
            ...
            # Let's write the performance of the away team when it plays away
            cur_round.loc[idx, ['Wins_When_Away']] = int(row['Win_Away'])
            cur_round.loc[idx, ['Draw_When_Away']] = int(row['Draw_Away'])
            cur_round.loc[idx, ['Lose_When_Away']] = int(row['Lose_Away'])
            ...
            # Streak of the Away team only for the matches played at Away
            cur_round.loc[idx, ['Streak_When_Away']] = row['Streak_When_Away']
    ...
    for _, rows in cur_round.iterrows():
        ...
        # added to the right, so the leftmost character corresponds to the
        # first round, or the first round where the team was at home
        # if we are in the Streak_Home column
        ...
        df_standings.loc[df_standings['Team']
                         == rows['Home_Team'],
                         'Streak_When_Home'] += dict_streak[rows['Label']]
        ...
        df_standings.loc[df_standings['Team']
                         == rows['Away_Team'],
                         'Streak_When_Away'] += dict_streak[rows['Label']]
        ...
        df_standings.loc[df_standings['Team']
                         == rows['Home_Team'],
                         'Goals_For'] += rows['Goals_For_Home']
        df_standings.loc[df_standings['Team']
                         == rows['Home_Team'],
                         'Goals_For_When_Home'] += rows['Goals_For_Home']
        df_standings.loc[df_standings['Team']
                         == rows['Home_Team'],
                         'Goals_Against'] += rows['Goals_For_Away']
        df_standings.loc[df_standings['Team']
                         == rows['Home_Team'],
                         'Goals_Against_When_Home'] += rows['Goals_For_Away']
        # Let's see the away team
        df_standings.loc[df_standings['Team']
                         == rows['Away_Team'],
        ...
        df_standings.loc[df_standings['Team']
                         == rows['Away_Team'],
                         'Goals_For_When_Away'] += rows['Goals_For_Away']
        df_standings.loc[df_standings['Team']
                         == rows['Away_Team'],
                         'Goals_Against'] += rows['Goals_For_Home']
        df_standings.loc[df_standings['Team']
                         == rows['Away_Team'],
                         'Goals_Against_When_Away'] += rows['Goals_For_Home']
